...
def filter_images(input_pattern, pattern, width, height):
    ...
    images_path = path.dirname(path.abspath(input_pattern))
    corners_path = f'{images_path}/corners'
    Path(corners_path).mkdir(parents=True, exist_ok=True)

    to_del = []
    for i, fname in enumerate(images):
        #print fname
        img_name = Path(fname).resolve().stem
...
def main():
    ...
    for i in range(len(opoints)):
        ...
        tot_error += error
    print(f"total error :, {tot_error}, mean error :, {tot_error/len(opoints)}")
